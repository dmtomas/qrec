...
def ProbabilityRandom(N, maximum, pos_max, val, pos_val, T, delta1=0.2, delta2=5):
    k = delta1 * (pos_max - pos_val) / (N * (1 + T))
    k2 = delta2 * (maximum - val) / (1 + T)
    #return np.exp(-1/T) / N + np.exp(-T) * np.exp(-(k2**2)) * np.exp(-(k**2))
    return np.exp(-(k**2))

def near_random(arr, ep):
    maximum = np.max(arr)
    pos_max = list(arr).index(maximum)
    
    prob = []
    T = 1 / (1 + np.exp(-30 * ep + 9))
    for i in range(len(arr)):
        prob.append(ProbabilityRandom(len(arr), maximum, pos_max, arr[i], i, T))
